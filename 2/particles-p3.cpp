...
struct AlloApp : App {
  Parameter pointSize{"/pointSize", "", 0.05, "", 0.0, 0.1};
  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < vertex.size(); i++) {
      for (int j = i+1; j < vertex.size(); j++) {
          // calculate force
          Vec3f forceionj = gravitationalForce(mass[i], mass[j], vertex[i], vertex[j]) / asymetryFactor;
          Vec3f forcejoni = gravitationalForce(mass[j], mass[i], vertex[j], vertex[i]) * asymetryFactor;
          // apply force
